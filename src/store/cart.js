...
const cartSlice = createSlice({
  ...
  reducers: {
    addCart(state, action) {
      //Tìm index sản phẩm trong giỏ nếu đã tồn tại
      const cartItemExistIndex = state.listCart.findIndex(
        (cart) => cart.id === action.payload.id
      );
      //Không tồn tại trong giỏ
      if (cartItemExistIndex === -1) {
    ...
    setEmptyCart(state) {
      //Đặt giỏ hàng về rỗng
      state.listCart = [];
    },
  },
});

//Xuất các actions của giỏ hàng
export const cartActions = cartSlice.actions;

//Xuất reducer của cart slice
export default cartSlice.reducer;
